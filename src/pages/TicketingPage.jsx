...
export default class TicketingPage extends Component {
	...
	triggerChildAlert = () => {
		...
		var url =
			"https://cors-anywhere.herokuapp.com/54.159.114.209:211/datasnap/rest/TServerMethods1/Bilhetagem?70EF42";
	...
	somaValorSemEconomia() {
		...
		let total = this.state.bilhetagemJson.reduce(
			(total, valor) => total + valor.valsemeconomia,
			0
		);
	...
	somaValorComEconomia() {
		...
		let state = this.state;
		let total = this.state.bilhetagemJson.reduce(
			(total, valor) => total + valor.valcomeconomia,
			0
		);
		state.bilhetagemJson = [];
	...
	componentDidMount() {
		...
		fetch(
			"https://cors-anywhere.herokuapp.com/54.159.114.209:211/datasnap/rest/TServerMethods1/Usuarios?70EF42",
			requestOptions
		)
			.then((response) => response.json())
		...
		fetch(
			"https://cors-anywhere.herokuapp.com/54.159.114.209:211/datasnap/Rest/tserverMethods1/Impressoras?70EF42",
			requestOptions
		)
			.then((response) => response.json())
		...
		fetch(
			"https://cors-anywhere.herokuapp.com/54.159.114.209:211/datasnap/Rest/tserverMethods1/Departamentos?70EF42",
			requestOptions
		)
			.then((response) => response.json())
			...
			<div className="ticketing-page">
				...
				<div className="ticketing-filters">
					...
					<div id="printer-filter" className="filters">
						<select
							value={this.state.impCodigo}
							onChange={this.handleChangeImp}
						>
					...
					<div id="dpt-filter" className="filters">
						<select
							value={this.state.dptCodigo}
							onChange={this.handleChangeDpt}
						>
						...
						<div className="date-select-container">
							<div className="date-filters" id="init-date-filter">
								<input
									...
									onChange={this.handleChangeDataIni}
								/>
							</div>
					...
					<button className="filter-button" onClick={this.triggerChildAlert}>
						Filtrar
					</button>
				</div>
				{/* FIM DOS FILTROS */}
				...
				<div className="under-table-container">
					...
					<div className="under-table-container-left">
						<div style={{ textAlign: "left", padding: "10px 30px" }}>
							<form>
								<label>
									<span style={{ color: "#5babdf", fontWeight: "500" }}>
										Economia:
									</span>{" "}
									<strong>Fora do sistema </strong>
