			...
			<div className="screen-graficos">
				<div className="filtro">
					...
					<input
						type="datetime-local"
						value={inicial}
						onChange={(e) => this.setState({ inicial: e.target.value })}
					/>
					<input
						type="datetime-local"
						value={final}
				...
				<div className="areaGrafico">
					...
					<div>{this.state.apt ? <Grafico4 /> : ""}</div>
					<div>
						{this.state.apt ? <Grafico1 inicio={dInicio} final={dFinal} /> : ""}
					</div>
					<div>{this.state.apt ? <Grafico3 /> : ""}</div>
					<div>{this.state.apt ? <Grafico2 /> : ""}</div>
				</div>
			</div>
		);
	}
