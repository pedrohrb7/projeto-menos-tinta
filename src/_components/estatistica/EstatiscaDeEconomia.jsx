...
class EstatiscaDeEconomia extends Component {
	...
	componentDidMount() {
		...
		fetch(
			"https://cors-anywhere.herokuapp.com/54.159.114.209:211/datasnap/rest/TServerMethods1/ConsolidadoEconomiaGeral?E754",
			requestOptions
		)
			...
			.then((result) => {
				...
				for (let item in result) {
					state.semEconomia[0].valores = result[item].custosemeconomia.toFixed(
						2
					);
					...
					state.series[0].data[0].y = parseFloat(
						result[item].economizado.toFixed(2)
					);
					state.series[0].data[1].y = parseFloat(
						result[item].custoreal.toFixed(2)
					);
					this.setState(state);
					...
					{this.state.semEconomia.map((item) => {
						return (
							<div className="valores">
								...
								<p className={item.dados}>R${item.valores}</p>
							</div>
						);
					})}
				</div>
				<div id="atmospheric-composition" className="grafico"></div>
			</>
		);
	}
}
